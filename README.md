...
- **自定义题单**：创建属于你的训练计划，支持右键快速添加题目。
- **一键导入/导出**：支持 JSON 格式的数据备份与恢复，同时也支持文本批量导入题目。

### 📊 状态追踪 & 比赛
- **实时状态同步**：输入 Handle 即可同步做题记录。
  - ✅ **AC** (绿色)
  - ⚠️ **Attempted** (黄色)
  - ⬜ **Unsolved** (灰色)
- **难度可视化**：题目分数显示为 CF 官方对应的段位颜色（如红名、紫名颜色）。
- **比赛日历**：查看即将开始的比赛（倒计时）以及个人的历史参赛记录。
