...
### 🤖 AI 排行榜 (New!)
- **实时更新排行**：实时展示主流 AI 模型的排行榜，包括 GPT-4、Claude、Gemini 等 20+ 主流模型。
- **多维度指标**：
  - **使用率 (Usage Rate)**：模型的实际使用频率
  - **热度 (Popularity)**：社区关注度和讨论热度
  - **性价比 (Cost-Effectiveness)**：综合性能与价格的性价比评分
  - **评分 (Rating)**：综合评分 (0-10 分)
- **智能筛选排序**：支持按指标排序，支持按类别筛选（LLM、代码助手、图像生成等）
- **自动刷新**：每 30 秒自动更新数据，保持信息实时性
- **可视化展示**：直观的进度条和排名展示，前三名特殊标记
